...
import {
	...
	IGroup,
	IGroupInput
} from '@/src/shared/types/group.types'
...
const ProductForm = ({ group }: GroupFormProps) => {
	...
	const onSubmit: SubmitHandler<IGroupInput> = data => {
		if (
			!(
				new Date(`1970-01-01T${data.endTimeLearning}`) >
				new Date(`1970-01-01T${data.startTimeLearning}`)
			)
		) {
								...
								<FormItem>
									...
									<FormControl>
										<Input
											...
											disabled={isLoadingCreateGroup || isPendingGroup}
											{...field}
										/>
									</FormControl>
									<FormMessage />
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Select
													disabled={isPendingGroup || isLoadingCreateGroup}
													value={field.value}
