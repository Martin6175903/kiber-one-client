    ...
    <div className="rounded-lg border shadow-sm">
      <Table className={'overflow-hidden'}>
        <TableHeader className="bg-gray-50/50">
          <TableRow className={'grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_0.6fr]'}>
            <TableHead className={'flex items-center ml-1.5 w-full'}>
              <span>Название группы</span>
            </TableHead>
            <TableHead className={'flex items-center ml-0.5 w-full justify-center'}>
              <span>Возраст</span>
            </TableHead>
            <TableHead className={'flex items-center ml-0.5 w-full justify-center'}>
              <span>Год</span>
        ...
        <TableBody>
          {groups!.map((group) => (
            <TableRow key={group.id} className="hover:bg-gray-50/50 grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr_0.6fr]">
              ...
              <TableCell className="text-sm flex justify-center items-center">{switchAgeCategory(group.ageCategory)}</TableCell>
              <TableCell className="text-sm flex justify-center items-center">{group.yearOfGroup}</TableCell>
              <TableCell className="text-sm flex justify-center items-center">{dayLesson(group.dayOfStudy)}</TableCell>
              ...
              <TableCell className={'flex justify-center items-center'}>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-40">
                    <DropdownMenuItem className="cursor-pointer" onSelect={() => router.push(PUBLIC_URL.admin(`/groups/${group.id}`))}>
                      <Pencil className="mr-2 h-4 w-4" />
                      Редактировать
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => {
                      deleteGroup(group.id!)
                    }} disabled={isLoadingDeleteGroup} className="cursor-pointer text-red-600">
