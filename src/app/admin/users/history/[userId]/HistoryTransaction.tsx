...
import {
	adminHistoryColumns,
	IAdminHistoryColumn
} from '@/src/app/admin/users/history/[userId]/AdminHistoryColumns'
...
const HistoryTransaction = () => {
	...
	const formattedHistory: IAdminHistoryColumn[] =
	...
	return (
		<div>
			...
			<p className={'text-xl mb-7'}>
				{group ? (
					<span>
						{user.name}, {group.title}, {dayLesson(group.dayOfStudy)},{' '}
						{group.startTimeLearning} - {group.endTimeLearning}
					</span>
				) : (
					<span>{user.name} (Группа не указана)</span>
				)}
			</p>
			<div className={'flex justify-between items-center mb-4'}>
				<p className={'text-base font-medium mb-2'}>
					Текущий баланс ученика:{' '}
					<span className={'font-bold'}>{user.quantityMoney} K</span>
				</p>
