...
export const adminHistoryColumns: ColumnDef<IAdminHistoryColumn>[] = [
			...
			return (
				...
				>
					<span className={'text-left w-full flex items-center'}>
						Дата операции <ArrowUpDown className={'ml-2 size-4'} />
					</span>
				</Button>
			)
		},
		cell: ({ row }) => (
			<span className={'ml-5 text-left inline-block w-full font-medium'}>
				{format(new Date(row.getValue('dateOperation')), 'dd.MM.yyyy')}
			</span>
		)
	},
	{
		...
		cell: ({ row }) => (
			<span className={'ml-2 text-left inline-block w-full font-medium'}>
	...
	},
	{
		...
		cell: ({ row }) => (
			<span className={'font-medium'}>
				{row.original.type === EnumTypeTransaction.BONUS
					? 'Начисление'
					: 'Списывание'}
			</span>
		)
	...
	},
	{
		accessorKey: 'actions',
		header: () => {
			return <Button variant={'ghost'}>Действия</Button>
		},
		cell: ({ row }) => {
			const params = useParams<{ userId: string }>()
			const { deleteTransaction, isLoadingDeleteTransaction } =
