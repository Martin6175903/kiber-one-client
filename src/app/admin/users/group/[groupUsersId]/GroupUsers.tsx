...
const GroupUsers = () => {
	...
	const formattedUsers: IAdminUsersColumn[] = isLoadingUsers
		? []
		: (users!
				.filter(user => user.role && user.groupId === params.groupUsersId)
				.map((user, index) => ({
	...
	return (
		<div>
			...
			<InputDataUser
				users={users!.filter(
					user => user.role && user.groupId === params.groupUsersId
				)}
			/>
			<h4 className={'font-medium text-2xl'}>
				{isLoadingGroup ? '' : group!.title}
			</h4>
			<AdminDataTable
