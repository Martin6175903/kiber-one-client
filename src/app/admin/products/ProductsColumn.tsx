...
export const productsColumn: ColumnDef<IProductsColumn>[] = [
	...
	},
	{
		...
		cell: ({ row }) => <blockquote>{row.original.description}</blockquote>
	},
	{
		accessorKey: 'price',
		header: ({ column }) => {
		...
		cell: ({ row }) => <span>{row.original.price} K</span>
	},
	{
		accessorKey: 'isStock',
		header: ({ column }) => {
		...
		cell: ({ row }) => <span>{row.original.isStock ? 'Да' : 'Нет'}</span>
	},
	{
		accessorKey: 'size',
		header: ({ column }) => {
