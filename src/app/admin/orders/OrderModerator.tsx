...
const OrderModerator = () => {
	...
	return (
		<div className={'flex flex-col gap-5'}>
			{updateOrders.length ? (
				updateOrders.map((order, index) => (
					<div key={order.id}>
						<hr className={'my-2 h-1.5 bg-black/20 rounded-full'} />
						<div className={'flex justify-between items-center mb-3'}>
							<div>
								Сумма за заказ:{' '}
								<span className={'font-bold'}>{order.total} K</span>
							</div>
						...
						<div
							className={
								'grid grid-cols-3 gap-10 px-1 py-5 border-solid border-2 border-black/30 rounded-lg'
							}
						>
							{order.orderItems.map(item => (
								<div key={item.id} className={'flex items-center gap-3'}>
									...
									<div>
										...
										<p>
											Размер: <span className={'font-bold'}>{item.size}</span>
										</p>
										<p>
											Сумма за товар:{' '}
				...
				<div className={'text-2xl font-medium'}>
					<span
						className={
							'inline-block border-b-3 border-b-solid border-b-black/50'
						}
					>
