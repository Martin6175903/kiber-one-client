		...
		<div>
			<div className={'grid grid-cols-3 gap-10 mb-5'}>
				{items.map(item => (
					<div
						key={item.id}
						className={
							'bg-gray-500/15 flex flex-col gap-2 items-center text-center rounded-lg pt-2'
						}
					>
						<Link href={PUBLIC_URL.product(item.product.id)}>
							<img
								...
								className={'size-50 rounded-md duration-300 hover:scale-105'}
							/>
						</Link>
						<div
							className={
								'flex flex-col gap-4 items-center bg-darkyellow w-full text-white rounded-md py-2 px-1 text-[17px]'
							}
						>
							...
							<p className={'flex flex-col gap-1'}>
								...
								<span className={'font-bold'}>
									{Number(item.product.price) * item.quantity} K
								</span>
							</p>
						</div>
					</div>
			...
			{items.length > 0 ? (
				<p className={'text-center my-5 font-bold'}>
					Итоговая стоимость заказа: {total} K
				</p>
			) : (
				<div className={'text-xl font-medium flex flex-col gap-3'}>
					...
					<div>
						<Link
							className={
								'inline-block border-b-solid border-b-3 border-b-black/60 animate-pulse hover:animate-none duration-300 hover:scale-110'
							}
