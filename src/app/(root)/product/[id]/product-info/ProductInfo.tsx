...
const ProductInfo = ({ product }: ProductInfoProps) => {
	const [choiceSize, setChoiceSize] = useState(
		product?.size?.length ? product.size[0] : undefined
	)
	...
	return (
		<div className={'mt-10 space-y-5 sm:mt-16 lg:mt-0 [&>hr]:my-4'}>
			...
			{product.size?.length ? (
				<div
					className={
						'*:cursor-pointer *:hover:bg-amber-50/5 *:duration-500 mb-5 flex gap-5 items-center'
					}
				>
					<span>Доступные размеры: </span>
					{product.size?.length === 1 ? (
						<span>{choiceSize}</span>
					) : (
						<Select
							defaultValue={product.size[0]}
							onValueChange={value => setChoiceSize(value)}
						>
			...
			<div className={'flex gap-5 items-center'}>
				...
				<div className={'flex items-center mt-1'}>
					...
					<input
						...
						disabled
						readOnly
						value={storeItem ? storeItem.quantity : quantity}
					/>
			...
			<div>
				<AddToCartButton
					product={product}
					quantity={storeItem ? storeItem.quantity : quantity}
					size={choiceSize !== undefined ? choiceSize : undefined}
				/>
			</div>
		</div>
	)
