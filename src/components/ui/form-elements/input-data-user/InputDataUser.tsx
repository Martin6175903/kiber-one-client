...
const InputDataUser = ({users}: InputDataUserProps) => {
  ...
  return (
    <div className={'flex flex-col gap-4 mt-3 mb-8'}>
      <Label htmlFor={'input-data-user'} className={'text-xl font-bold'}>Зачисление пользователю по номеру карты:</Label>
      <InputOTP pattern={REGEXP_ONLY_DIGITS} ref={searchInputRef} id={'input-data-user'} aria-label={'input-otp'} maxLength={10} onKeyDown={(e) => {
        if (e.key === 'Enter') {
          const user = users?.find(user => String(user?.numberCard) === searchInputRef?.current?.value)
          if (!user) {
