	...
	return (
		<div className='flex items-center mb-5 relative bg-gray-100/10 rounded-md px-1 py-2'>
			<Link
				...
				className={'relative size-30 rounded-md overflow-hidden'}
			>
				<Image
			...
			<div className='ml-6'>
				...
				<button
					className={
						'mt-2 text-sm border-solid border-white border rounded-full py-2 px-3 duration-300 hover:text-darkyellow hover:border-darkyellow cursor-pointer'
					}
					onClick={() => removeFromCard({ id: item.id })}
				>
					Удалить из корзины
				</button>
			</div>
