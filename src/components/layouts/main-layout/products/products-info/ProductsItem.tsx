...
const ProductsItem = ({product} :ProductsItemProps ) => {
 ...
 return (
  <div className={'flex flex-col gap-5 text-center bg-gray-800/30 rounded-b-lg rounded-t-xl duration-400 hover:scale-[1.03]'}>
    <Link href={`product/${id}`} className={'max-[1200px]:flex max-[1200px]:justify-center relative'}>
      {!isStock && (
        <div className={'bg-red-500/70 py-2 px-4 rounded-xl absolute text-xl animate-pulse'}>Предзаказ</div>
      )}
      <img src={images[0]} alt={title} className={'size-[215px] md:w-[385px] md:h-[400px] rounded-t-xl object-cover'}/>
    </Link>
